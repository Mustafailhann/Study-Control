...
async function buildUserSummaryForReport({ db, userType, uid }) {
    ...
    let recentMockExams = [];
    if (uid) {
        const mockSnap = await db.collection("mock_exams")
            .where("uid", "==", uid)
            .orderBy("date", "desc")
            .limit(3)
            .get();
        recentMockExams = mockSnap.docs.map(d => d.data());
    }
...
`.trim();
        ...
        const docRef = await db.collection("ai_reports").add(docData);
        ...
        return json(200, { ok: true, report: createdReport });
    } catch (e) {
